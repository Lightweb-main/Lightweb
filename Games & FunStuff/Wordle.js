...
function generate(){
	for(let i = 0; i < numberOfGuesses; i++){
		...
		for(let j = 0; j < 5; j++){
			...
			let letterSquareInnerDiv = document.createElement("div");
			
			
			
			letterSquare.className = "letter";
			letterSquareInnerDiv.className = "letterInnerDiv";
			letterSquare.appendChild(letterSquareInnerDiv);
...
function insertLetter(pressedKey){
	...
	let box = row.children[nextLetter].children[0];
...
function deleteLetter(){
	let row = document.getElementsByClassName("rows")[6 - guessesRemaining];
	let box = row.children[nextLetter - 1].children[0];
	box.textContent = "";
...
function checkGuess(){
	...
	for(let i = 0; i < 5; i++){
		...
		let box2 = row.children[i].children[0];
