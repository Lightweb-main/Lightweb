 ...
 const TOTAL = 7;
    ...
    function newQuestion() {
	  ...
	  AdditionSubtractionToggle = Math.floor(Math.random() * 2);
	  if(AdditionSubtractionToggle >= 0){
      qVisual.textContent = `What is ${number1} + ${number2}?`;
	  }
	  if(AdditionSubtractionToggle >= 1){
      qVisual.textContent = `What is ${number1} - ${number2}?`;
	  if(number2 > number1){
	  numberReroll();
	  }
    ...
    function endQuiz() {
		Percent = ((correctAnswers / TOTAL) * 100).toFixed(0);
		AverageTime = (runTime / TOTAL).toFixed(2);
		let runTimeDisplay = runTime.toFixed(2)
		if(Percent <= 50){
		LetterGradeV2 = "F-"
		}
		if(Percent >= 55 ){
      ...
      CorrectAnswers.textContent = `You correctAnswersd ${correctAnswers} / ${TOTAL}`;
	  ...
	  TimePerQuestion.textContent = `Time Per Question: ${AverageTime} sec per Input (Average)`;
	}
